...
} from "../src/editor/compilePreview.js";
...
test("createSdnFlowEditorCompilePreview returns the canonical compile plan", async () => {
  const result = await createSdnFlowEditorCompilePreview([
    {
      id: "tab-1",
      type: "tab",
      label: "Preview Flow",
  ...
  assert.equal(result.outputName, result.compilePlan.outputName);
  assert.equal(result.source, result.compilePlan.source);
  assert.equal(result.command, result.compilePlan.command);
  assert.equal(result.runtimeModel, result.compilePlan.runtimeModel);
  assert.equal(result.sourceGeneratorModel, result.compilePlan.sourceGeneratorModel);
  assert.equal(result.compilePlan.outputName, result.outputName);
  assert.match(result.compilePlan.source, /generated by the native sdn-flow C\+\+ source generator/);
});

test("createSdnFlowEditorCompilePreview rejects alternate compile seams", async () => {
  await assert.rejects(
    ...
    /Preview compilation no longer accepts compiler overrides/,
  );
  ...
  await assert.rejects(
    ...
    /Preview compilation no longer accepts sourceGenerator overrides/,
  );
});
