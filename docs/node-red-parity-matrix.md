...
| `change`, `switch`, `range`, `template`, `json`, `csv`, `yaml`, `xml`, `html`, `split`, `join`, `batch`, `sort`, `link in`, `link out`, `link call` | standalone `wasi` | JS runtime | Shipped in `src/editor/nodeRedRegistry.generated.js`; runtime handlers in `src/editor/runtimeManager.js`; covered by `test/editor-runtime-nodes.test.js` and `test/editor-runtime-manager.test.js`. |
